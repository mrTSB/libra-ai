...
export default function UserMessage({
  ...
  // Show only the user's prompt, hide appended document context
  const marker = "Current document content";
  const markerIndex = message.indexOf(marker);
  const visibleMessage = markerIndex >= 0 ? message.slice(0, markerIndex).trimEnd() : message;
  return (
    <div
      className={cn(
    ...
    >
      ...
      <p className="px-1">{visibleMessage}</p>
